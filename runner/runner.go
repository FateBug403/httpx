...
import (
	...
	customport "github.com/FateBug403/httpx/common/customports"
	fileutilz "github.com/FateBug403/httpx/common/fileutil"
	"github.com/FateBug403/httpx/common/httputilz"
	"github.com/FateBug403/httpx/common/httpx"
	"github.com/FateBug403/httpx/common/slice"
	"github.com/FateBug403/httpx/common/stringz"
	...
	//wappalyzer "github.com/projectdiscovery/wappalyzer"
...
type Runner struct {
	options         *Options
	hp              *httpx.HTTPX // 发起请求的核心功能
	wappalyzer      *wappalyzer.Wappalyze // 指纹识别
...
func New(options *Options) (*Runner, error) {
	...
	//	//customport.CustomPorts = options.CustomPorts
	//}

	return runner, nil
}

// prepareInputPaths 检查在每个请求后添加的path路径
func (r *Runner) prepareInputPaths() {
	// 最可能的情况是，用户将提供现有文件的最简化路径
...
func (r *Runner) prepareInput() {
	...
	if len(r.options.InputFile)>0 { // 检查目标文件是否存在
		numHosts, err = r.loadAndCloseFile(r.options.InputFile)
		if err != nil {
...
//}
// loadAndCloseFile 批量加载目标
func (r *Runner) loadAndCloseFile(targets []string) (numTargets int, err error) {
	for _,target := range targets{ // 读取文件每一行
...
// RunEnumeration 开始扫描
func (r *Runner) RunEnumeration(reqResult resultFunc) {
	// 保存http响应
	...
	wgoutput := sizedwaitgroup.New(1)
	wgoutput.Add()
	// 处理输出结果,output是保存更底层调用的返回值，reqResult是作为RunEnumeration的返回值
	output := make(chan Result)
	go func(output chan Result) {
		defer wgoutput.Done()
		...
		if r.options.Screenshot { //创建截图索引文件
			var err error
			indexScreenshotPath := filepath.Join("uploads", "screenshot", "index_screenshot.txt")
			if r.options.Resume {
...
retry:
	...
	isCDN, cdnName, err := hp.CdnCheck(ip) // CDN检测
	...
	// 如果是域名访问提取域名，如果是IP记录真实IP
	var RealityIP string
	var Domain string
	checkIP :=net.ParseIP(parsed.Hostname())
	if checkIP == nil{
	...
	if scanopts.Screenshot {
		...
		//screenshotBytes, headlessBody, err = r.browser.ScreenshotWithBody(fullURL, 15*time.Second)
		//log.Println(string(screenshotBytes))
		if err != nil {
			screenshotPath="" // 如果发生错误，置截图路径为空
			...
			if err != nil {
				...
				// 将图片的绝对路径转化为相对路径返回
				index := strings.Index(screenshotPath, "uploads")
				if index == -1 {
					screenshotPath="" // 如果发生错误，置截图路径为空
					fmt.Println("Keyword not found in path")
				}else {
					screenshotPath = screenshotPath[index:]
				}
			}
		}
	}
